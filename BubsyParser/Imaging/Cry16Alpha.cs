...
{
	internal struct Cry16Alpha : IPixel<Cry16Alpha>
	{

		public byte Color { get; set; }
		public byte Intensity { get; set; }

		public Cry16Alpha(byte color, byte intensity) => (Color, Intensity) = (color, intensity);

		public readonly Vector3 ToVector3()
		{
			return new Vector3(
				CrToRed[Color] * Intensity / (float)byte.MaxValue / (float)byte.MaxValue,
				CrToGreen[Color] * Intensity / (float)byte.MaxValue / (float)byte.MaxValue,
				CrToBlue[Color] * Intensity / (float)byte.MaxValue / (float)byte.MaxValue
			);
		}

		public Vector4 ToVector4() => new(this.ToVector3(), Color > 0 || Intensity > 0 ? 1f : 0f);
		...
		public readonly PixelOperations<Cry16Alpha> CreatePixelOperations() => new PixelOperations();
		...
		public bool Equals(Cry16Alpha other) => Color == other.Color && Intensity == other.Intensity;

		public override bool Equals([NotNullWhen(true)] object? other) => other is Cry16Alpha pixel && Equals(pixel);

		public override int GetHashCode() => HashCode.Combine(Color, Intensity);

		public static bool operator ==(Cry16Alpha left, Cry16Alpha right) => left.Equals(right);

		public static bool operator !=(Cry16Alpha left, Cry16Alpha right) => !left.Equals(right);


		internal class PixelOperations : PixelOperations<Cry16Alpha>
		{
